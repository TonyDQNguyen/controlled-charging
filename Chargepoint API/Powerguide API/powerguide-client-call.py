...
## powerguide_client_call() queries data using SolarCity Powerguide API and
## return an array of timestamps, solar generation measurements, a float of
## the latest newest_timestamp and latest solar generation newest_measurement
...
##    print 'Obtained Access Token'
    ...
    # Request Customer detail for customer records:
    # NOTE: ['Data'][n]['GUID'] = different locations
    # n = 0: Ryan Harty's House ; n = 1 :CT Facility ; n = 2: Chino Parts Center..
    # ..on Monte Vista ; n = 3: Chino Parts Center on Yorba Ave
...
## metrics() takes in an amount of electricity_in_kWh and calculate the
## amount of carbon dioxide offset and trees seedling with a 10 year lifespan
## saved using equations given by EPA's Energy Star environmental equivalency
## calculator
...
## update_line() is the method used by the plot animation function in the main
## method. It calls powerguide_client_call() to receive latest solar data then
## call metrics() to calculate the newest daily totals environmental figures
## then redraw the plot
def update_line(i):
    timestamps, measurements, newest_timestamp, newest_measurement = powerguide_client_call()
    carbon_offset, trees_saved = metrics(newest_measurement)
    global total_trees_saved
    global total_carbon_offset
    global n
    if n != len(measurements):
        total_carbon_offset += carbon_offset
        total_trees_saved += trees_saved
        n+=1
    timestamps_hour = []
...
## Main method call powerguide_client_call() then metrics() to initate the
## data necessary to plot, then plot the solar generation graph over the span
## of the day in intervals of 15 minutes
if __name__ == "__main__":
    try:
        ...
        rects = ax.bar(timestamps_hour,measurements, color = 'green', width = 0.15)
        line_ani = animation.FuncAnimation(fig1, update_line, interval = 200)
        plt.xlabel('Time in hour')
        plt.ylabel('Solar-generated energy in kWh')
        plt.xlim(0,24)
        plt.xticks(numpy.arange(0,24 , 1.0))
